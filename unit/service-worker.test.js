...
const CACHE_NAME = "togs-heads-up-v11";
...
test("install precaches index and every scoped JS/CSS asset before taking control", async () => {
  ...
  assert.ok(harness.cacheStores.has("togs-heads-up-v10"), "old cache remains until activate");

  await harness.dispatchExtendable("activate");

  assert.deepEqual(harness.deletedCaches, ["togs-heads-up-v10"]);
...
test("a failed hashed asset prevents activation and preserves the previous cache", async () => {
  ...
  harness.seedCache("togs-heads-up-v10", [[INDEX_URL, new Response("old offline shell")]]);

  await assert.rejects(harness.dispatchExtendable("install"), /Failed to cache.*HTTP 503/);

  assert.ok(harness.cacheStores.has("togs-heads-up-v10"));
