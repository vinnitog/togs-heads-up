...
- A geolocalização é opcional. Se ativada, coordenadas exatas são enviadas ao Open-Meteo e à OpenWeather configurada no app; em ambientes sem chave, o nome do local é obtido pelo BigDataCloud. Coordenadas exatas não são persistidas no armazenamento do app.
- OpenWeather carrega automaticamente com a chave do projeto, sem pedir uma chave ao visitante. Essa chave integra o JavaScript público e o shell offline. Abrir mapas adiciona consultas de tiles à OpenWeather e OpenStreetMap; as chamadas autenticadas enviam a chave somente à OpenWeather.
...
- **Destinatários atuais**: Open-Meteo e OpenWeather configurada no app (nome do local, clima e ar); em ambientes sem chave, BigDataCloud (nome do local). BigDataCloud deve ser removido/regularizado antes da publicação desta política; OpenWeather ainda precisa de avaliação dos papéis, termos e retenção.

### 2.2 Busca de cidade e previsão

- **Dados**: texto pesquisado, cidade ou CEP/país, coordenadas da localidade e metadados técnicos; chave compartilhada do projeto nas consultas OpenWeather. Não é coletada chave do visitante.
...
- **Destinatários atuais**: Open-Meteo, OpenWeather na busca de cidade/CEP e BrasilAPI, que distribui a previsão pública do CPTEC/INPE quando aplicável. A LIA precisa incorporar o novo destinatário e a busca por CEP.
...
- **Destinatários/fontes atuais**: Open-Meteo, OpenWeather, INMET, RSS2JSON, BrasilAPI, CPTEC/INPE, AllOrigins e NASA/JPL. G1 e Giro Marília fornecem feeds buscados pelo RSS2JSON; AllOrigins é usado somente para JPL e não recebe a chave OpenWeather.
- **Mapas opcionais**: ao abrir a aba Mapas, OpenWeather e OpenStreetMap recebem índices dos tiles da região visualizada e metadados de rede. As chamadas autenticadas enviam a chave do projeto apenas à OpenWeather. A avaliação desses destinatários e a atualização da LIA permanecem pendentes.
- O app não cria perfis das pessoas eventualmente citadas nas notícias.

### 2.4 Cache funcional/PWA

- **Dados**: assets, incluindo JavaScript com chave OpenWeather do projeto, timestamps, clima/localidade em nível de cidade, previsão CPTEC e eventos espaciais públicos; respostas OpenWeather somente em memória.
...
| shell PWA, incluindo JavaScript com a chave do projeto | Cache Storage `togs-heads-up-v17` | até atualização ou limpeza do navegador |
...
| chave OpenWeather do projeto | configuração local/GitHub Secret; injetada no JavaScript público durante o build e incluída no shell | configuração até remoção/rotação; cópias publicadas e locais até substituição/limpeza |
| respostas/URLs OpenWeather, inclusive coordenadas e chave | memória do navegador, sem `localStorage` ou Cache Storage | TTL lógico de 10 min, até 80 entradas; descarte na atualização/recarregamento |
...
| OpenWeather | geocodificação, clima, qualidade do ar e tiles meteorológicos; integração automática com chave do projeto | pendente de avaliação | novo destinatário; termos, papéis, logs e fluxos pendentes |
...
O app não cria cookies de analytics, publicidade ou rastreamento. Usa `localStorage` e Cache Storage para as finalidades funcionais descritas. A chave OpenWeather do projeto integra o JavaScript público e o shell offline, sem armazenamento separado no `localStorage`. Valores antigos da configuração de chave em `sessionStorage` são ignorados; não há garantia de eliminação dessas entradas legadas pelo app. Provedores acessados e links externos possuem políticas próprias.
...
O app usa HTTPS e allowlist de hosts, não persiste coordenadas exatas e não inclui respostas de APIs externas no Cache Storage. O GitHub Secret `OPENWEATHER_API_KEY` é injetado como `VITE_OPENWEATHER_API_KEY` no build; a chave compartilhada fica visível no JavaScript público, nas chamadas do navegador e no shell offline. A configuração real não é versionada no código-fonte. As chamadas autenticadas seguem diretamente aos domínios oficiais OpenWeather, sem proxy público. Mudanças são testadas por CI. Existe runbook em `.lgpd/incidents/`.
...
- Adendo técnico — 11/09/2026: integração automática OpenWeather com chave do projeto no JavaScript/shell, CEP, mapas OpenStreetMap e retenção das respostas; mantida a condição de minuta não vigente, sem aprovação jurídica nova.
